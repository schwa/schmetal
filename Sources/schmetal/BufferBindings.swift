...
struct BufferBindings {
    ...
    init(parameters: [ASTNode], automaticResources: Set<String>, nonBindable: Set<String>, isEntryPoint: Bool) throws {
        ...
        for parameter in parameters {
            guard let name = parameter.name else { throw SchmetalError("unnamed binding parameter") }
            guard let slot = try Self.explicitSlot(parameter, name: name) else { continue }
            guard isEntryPoint, !nonBindable.contains(name) else {
                throw SchmetalError("@buffer requires a resource parameter on an entry point: '\(name)'")
            }
            guard used.insert(slot).inserted else { throw SchmetalError("duplicate buffer slot \(slot)") }
            slots[name] = slot
        }
        ...
        for parameter in parameters {
            ...
            guard let slot = Self.validSlots.first(where: { !used.contains($0) }) else {
                throw SchmetalError("entry point requires more than 31 buffer bindings")
            }
            slots[name] = slot
            used.insert(slot)
        }
    ...
    private static func explicitSlot(_ parameter: ASTNode, name: String) throws -> Int? {
        ...
        let invalid = SchmetalError("@buffer on '\(name)' requires an integer literal from 0 through 30")
